...
Usage:
	...
	実験棟
	perl oto18_Ntfy_Upper_Lower.pl xfel_bl_3_tc_bm_1_pd/charge 93 1000 0.02 oto0.wav 3 yes
...
my $wav_file    =   $ARGV[4];
my $cnt_limit   =   $ARGV[5];
my $flg_check_same_value   =   $ARGV[6]; #データ収集が停止してしまって、同じ値が続いている場合は、音を鳴らしてスキップするかどうか 0:しない 1:する
...
############################### #データ収集が停止してしまって、同じ値が続いている場合は、音を鳴らしてスキップする
	if($flg_check_same_value eq "yes"){
		if($mean_before == $result[3]){
			print  "Same value as before, skip the rest of the loop.\n";
